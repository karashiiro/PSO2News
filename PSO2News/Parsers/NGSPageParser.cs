...
{
    public class NGSPageParser : NewsPageParser
    {
        public override async IAsyncEnumerable<NewsInfo> GetNews(
            DateTime after,
            [EnumeratorCancellation] CancellationToken token)
        {
            var curUrl = "https://new-gen.pso2.jp/cbt/players/news/";
            do
            {
                var web = new HtmlWeb();
                var page = await web.LoadFromWebAsync(curUrl, token);

                var nextButton = page.DocumentNode.SelectSingleNode("//li[@class='pager__next']/a");
                curUrl = nextButton?.GetAttributeValue("href", null);

                var list = page.DocumentNode.SelectSingleNode("//ul[@class='newslist']");
